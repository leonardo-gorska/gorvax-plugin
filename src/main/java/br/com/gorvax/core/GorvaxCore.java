...
public class GorvaxCore extends JavaPlugin {
    ...
    private BukkitTask autosaveTask; // B44 — Autosave periódico
    ...
    public void onEnable() {
        ...
        getLogger().info("=========================================");
        getLogger().info(" GORVAX PLUGIN v" + version + " ");
        getLogger().info("=========================================");
        getLogger().info("[GorvaxCore] Inicializando sistema v" + version + "...");
        ...
        // B44 — Autosave Periódico Global (Crash Safety)
        setupAutosave();
    ...
    public void onDisable() {
        // B44 — Cancelar autosave antes de salvar manualmente
    ...
    private void setupBStats() {
        // Plugin registrado em https://bstats.org/ (ID: 30054)
        int pluginId = 30054;
    ...
    private void setupAutosave() {
        boolean enabled = getConfig().getBoolean("autosave.enabled", true);
        if (!enabled) {
            getLogger().info("[GorvaxCore] Autosave desativado via config.");
            return;
        }

        int intervalMinutes = getConfig().getInt("autosave.interval_minutes", 5);
        long intervalTicks = intervalMinutes * 60L * 20L; // Converter minutos para ticks

        autosaveTask = getServer().getScheduler().runTaskTimerAsynchronously(this, () -> {
            long start = System.currentTimeMillis();

            // Salvar todos os managers com persistência YAML
            try {
                if (claimManager != null) claimManager.saveSync();
                if (playerDataManager != null) playerDataManager.saveAll();
                if (kingdomManager != null) kingdomManager.saveSync();
                if (marketManager != null) {
                    marketManager.saveMarketData();
                    marketManager.saveLocalMarketSync();
                }
                if (auditManager != null) auditManager.saveSync();
                if (auctionManager != null) auctionManager.saveSync();
                if (priceHistoryManager != null) priceHistoryManager.saveSync();
                if (mailManager != null) mailManager.saveSync();
                if (voteManager != null) voteManager.saveSync();
                if (bountyManager != null) bountyManager.saveSync();
                if (nationManager != null) nationManager.saveSync();
                if (structureManager != null) structureManager.saveSync();
            } catch (Exception e) {
                getLogger().warning("[GorvaxCore] Erro durante autosave: " + e.getMessage());
            }

            long elapsed = System.currentTimeMillis() - start;
            getLogger().info(messageManager.get("admin.autosave_complete", elapsed));
        }, intervalTicks, intervalTicks);

        getLogger().info("[GorvaxCore] Autosave ativado (intervalo: " + intervalMinutes + " minutos).");
    }
