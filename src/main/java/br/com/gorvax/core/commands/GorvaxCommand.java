...
public class GorvaxCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleAuditCommand(CommandSender sender, String[] args) {
        ...
        if (audit == null) {
            sender.sendMessage(msg.get("admin_audit.not_initialized"));
            return;
        }
    ...
    private void handleMigrateCommand(CommandSender sender, String[] args) {
        var msg = plugin.getMessageManager();
        if (args.length < 3) {
            sender.sendMessage(msg.get("admin_migrate.usage"));
            sender.sendMessage(msg.get("admin_migrate.usage_types"));
            sender.sendMessage(msg.get("admin_migrate.usage_example"));
            return;
        }
        ...
        try {
            ...
            sender.sendMessage(msg.get("admin_migrate.invalid_type"));
            return;
        }

        if (sourceType == targetType) {
            sender.sendMessage(msg.get("admin_migrate.same_type"));
            return;
        }

        sender.sendMessage(msg.get("admin_migrate.starting", sourceType, targetType));
        sender.sendMessage(msg.get("admin_migrate.starting_hint"));
        ...
        org.bukkit.Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
                    sender.sendMessage(msg.get("admin_migrate.success"));
                    sender.sendMessage(msg.get("admin_migrate.success_hint"));
                });
            } catch (Exception e) {
                org.bukkit.Bukkit.getScheduler().runTask(plugin, () -> {
                    sender.sendMessage(msg.get("admin_migrate.error", e.getMessage()));
                    plugin.getLogger().log(java.util.logging.Level.SEVERE, "Erro na migração de storage", e);
                });
            }
        });
    }
    ...
    private void handleMigrateConfigCommand(CommandSender sender) {
        ...
        if (migrator.getCurrentVersion() >= migrator.getLatestVersion()) {
            sender.sendMessage(msg.get("admin_migrateconfig.already_updated", migrator.getCurrentVersion()));
            sender.sendMessage(msg.get("admin_migrateconfig.forcing"));
        }
        ...
        if (applied > 0) {
            sender.sendMessage(msg.get("admin_migrateconfig.success", applied));
            sender.sendMessage(msg.get("admin_migrateconfig.current_version", migrator.getCurrentVersion()));
        } else {
            sender.sendMessage(msg.get("admin_migrateconfig.no_pending"));
        }
    }
